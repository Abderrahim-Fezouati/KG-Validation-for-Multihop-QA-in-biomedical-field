        ...
        for line in f:
            ...
            for s in req:
                ...
                if s2 and s2.lower() not in seen:
                    seen.add(s2.lower()); req_clean.append(s2)
...
def main():
    ...
    for qid, hits in cache:
        ...
        for k in args.ks:
            ...
            status = "complete" if len(found) == len(req) else ("zero" if len(found)==0 else "partial")
            totals[k][status] += 1
    ...
    with io.open(args.out_per_query, "w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["qid","k","query_text","required_total","found_count","status","missing"])
        writer.writeheader()
    ...
    with io.open(args.out_summary, "w", encoding="utf-8") as f:
        ...
        for k in args.ks:
            t = totals[k]
            comp = t["complete"]; part = t["partial"]; zero = t["zero"]
            pct = (comp / total_qs * 100.0) if total_qs else 0.0
            f.write(f"  @k={k}: complete={comp} ({pct:.1f}%)  partial={part}  zero={zero}\n")
