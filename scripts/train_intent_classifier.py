...
def main():
    ...
    ap.add_argument("--data_dir", required=True, help="Folder containing train.jsonl / val.jsonl / test.jsonl and label_map.json")
    ...
    ap.add_argument("--out_dir",    required=True, help="Output directory for model/checkpoints")
    ...
    ds_tok = ds.map(tok, batched=True, remove_columns=[c for c in ds["train"].column_names if c not in ("label","labels","id")])
