...
def main():
    ...
    keep = ("label","labels","id","question")
    ds_tok = ds.map(tokenize, batched=True, remove_columns=[c for c in ds["train"].column_names if c not in keep])
    ...
    # IMPORTANT for your env: use eval_strategy (not evaluation_strategy)
