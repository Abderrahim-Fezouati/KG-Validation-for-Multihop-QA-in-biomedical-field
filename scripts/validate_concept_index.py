...
def load_ids_from_kg(kpath):
    ...
    with kpath.open(encoding="utf-8-sig") as f:
        r=csv.DictReader(f)
        for row in r:
            ids.add(row.get("head")); ids.add(row.get("tail"))
    return {x for x in ids if x}

def looks_node_id(s): 
    return bool(re.match(r'^(drug_|disease_|gene_|chemical_|rxnorm:|chebi:|drugbank:)', (s or ""), re.I))
...
for path in [DR, DI]:
    s=sample(path)
    for r in s:
        kid=r.get("kg_id"); txt=r.get("text")
        if not looks_node_id(kid):
            print("FAIL: kg_id is not a node id:", kid, "in", path.name); ok=False
        if looks_node_id(txt):
            print("FAIL: text looks like a node id:", txt, "in", path.name); ok=False
