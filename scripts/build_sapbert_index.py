...
with open(args.kg, newline="", encoding="utf-8") as f:
    ...
    if rows and rows[0] and rows[0][0].lower() in ("h","head","source"):
        rows=rows[1:]
    for h,rel,t in rows:
        kg_nodes.add(h.strip()); kg_nodes.add(t.strip())
...
if not node2surfs:
    raise SystemExit("No surfaces found for KG nodes. Check your dict/overlay coverage.")
...
for i in tqdm(range(0, len(texts), args.batch), desc="Encoding"):
    ...
    vecs = model.encode(batch, batch_size=args.batch, show_progress_bar=False, convert_to_numpy=True, normalize_embeddings=True)
    embs.append(vecs)
