...
def _factoid_rephrase(q):
    ...
    alts = [
        lambda x: "Provide the mechanism for " + re.sub(r"^\s*(explain|describe)\s+(the\s+)?", "", x, flags=re.I),
        lambda x: "Mechanistically, " + re.sub(r"^\s*(explain|describe)\s+(the\s+)?", "", x, flags=re.I),
        lambda x: "Summarize " + re.sub(r"^\s*(explain|describe)\s+(the\s+)?", "", x, flags=re.I),
    ]
...
def _yesno_hedge(q):
    s = re.sub(r"^(does|do|is|are|can|should|could|would)\b", random.choice(HEDGE_STARTS), q, flags=re.I)
    return _ensure_qmark(s)

def _list_directive(q):
    s = re.sub(r"^\s*(which|what|list|name|give|show)\b.*", "Provide a list of relevant entities that satisfy the constraints.", q, flags=re.I)
    return _ensure_qmark(s)
...
def main():
    ...
    ap.add_argument("--per_class", type=int, default=30, help="number of seeds per class to transform")
    ...
    final = []
    for r in out:
        key = re.sub(r"\s+"," ", re.sub(r"[^a-z0-9 ]+"," ", r["question"].lower())).strip()
        if key not in seen:
            seen.add(key)
            final.append(r)
