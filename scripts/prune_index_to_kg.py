...
def load_kg_nodes():
    ...
    with KG.open(encoding="utf-8-sig", newline="") as f:
        ...
        if "head" in cols: hk = r.fieldnames[cols.index("head")]
        if "tail" in cols: tk = r.fieldnames[cols.index("tail")]
        for row in r:
            if hk and row.get(hk): ids.add(row[hk].strip())
            if tk and row.get(tk): ids.add(row[tk].strip())
    return ids
