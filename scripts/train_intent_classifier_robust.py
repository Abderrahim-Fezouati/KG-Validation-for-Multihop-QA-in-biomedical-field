...
from transformers import (
    ...
    DataCollatorWithPadding, Trainer, EarlyStoppingCallback
)
...
def read_label_map(dir_path):
    ...
    if p.exists():
        ...
        if isinstance(j, dict) and "label2id" in j: return {str(k): int(v) for k,v in j["label2id"].items()}
...
def main():
    ...
    files = {"train": str(Path(args.data_dir, args.train_file)),
    ...
    def tokenize(ex): return tok(ex["question"], truncation=True, max_length=args.max_length)
    cols_to_remove = [c for c in ds["train"].column_names if c not in ("labels","id","question")]
    ds_tok = ds.map(tokenize, batched=True, remove_columns=cols_to_remove)
    collator = DataCollatorWithPadding(tokenizer=tok)

    cfg = AutoConfig.from_pretrained(args.model_name, num_labels=len(label2id), id2label=id2label, label2id=label2id)
    if hasattr(cfg, "dropout"): cfg.dropout = 0.2
    if hasattr(cfg, "attention_dropout"): cfg.attention_dropout = 0.2
    if hasattr(cfg, "seq_classif_dropout"): cfg.seq_classif_dropout = 0.2

    model = AutoModelForSequenceClassification.from_pretrained(args.model_name, config=cfg)

    # class weights (upweight factoid)
    counts = np.bincount([ex["labels"] for ex in ds_tok["train"]], minlength=len(label2id))
    ...
    Path(args.out_dir, "label_map.json").write_text(json.dumps({"label2id": label2id, "id2label": id2label}, indent=2), encoding="utf-8")
