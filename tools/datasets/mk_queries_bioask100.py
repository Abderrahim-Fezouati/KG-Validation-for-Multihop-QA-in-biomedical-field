...
def pick_surface(eid, overlay):
    ...
    if not ss:
        core = re.sub(r"^(drug|disease|class|symptom|gene|protein)_", "", eid, flags=re.I)
        ss = [core.replace("_", " ")]
    # prefer longer, more specific surface occasionally
    ss_sorted = sorted(ss, key=len, reverse=True)
    return random.choice(ss_sorted[:min(len(ss_sorted), 3)] if len(ss_sorted) >= 3 else ss_sorted)
...
POP_CTX = [
    "in adult patients", "in elderly populations", "in pediatric cases",
    "among oncology patients", "in clinical settings", "in routine practice"
]
EVID_CTX = [
    "according to published studies", "based on clinical observations",
    "as reported in the literature", "in randomized trials",
    "per case reports", "per pharmacovigilance data"
]
COADMIN_CTX = [
    "during co-administration", "when used together", "during concurrent therapy",
...
with io.open(kg_csv, "r", encoding="utf-8", errors="ignore") as f:
    for row in csv.reader(f):
        ...
        if not (h and r and t): continue
        if r == "INTERACTS_WITH": IW.append((h, t))
        elif r == "ADVERSE_EFFECT": AE.append((h, t))

random.shuffle(IW); random.shuffle(AE)
IW = IW[:60]     # sample a bit extra to dedup later
AE = AE[:60]

def make_iw_q(h, t):
    Hs, Ts = cap(pick_surface(h, overlay)), cap(pick_surface(t, overlay))
    Y = random.choice(["", random.choice(EVID_CTX), random.choice(COADMIN_CTX), random.choice(POP_CTX)]).strip()
    Y = ("" if Y == "" else Y)
...
for (h, t) in IW:
    item = make_iw_q(h, t)
    if item["text"] in uniq: continue
    uniq.add(item["text"]); out_items.append(item)
    if len([x for x in out_items if x["tail1"]]) >= 50: break

for (h, t) in AE:
    item = make_ae_q(h, t)
    if item["text"] in uniq: continue
    uniq.add(item["text"]); out_items.append(item)
    if len([x for x in out_items if x["tail2"]]) >= 50: break
...
print("wrote", out_path, "n=", len(out_items[:100]),
      "IW=", sum(1 for x in out_items[:100] if x["tail1"]),
      "AE=", sum(1 for x in out_items[:100] if x["tail2"]))
