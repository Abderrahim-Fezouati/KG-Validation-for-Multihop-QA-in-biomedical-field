...
AE_PAT  = re.compile(r"(adverse|side[- ]?effect|safety|harm|untoward|complication|toxicit)", re.I)
DDI_PAT = re.compile(r"(avoid(ed)? with|clash|co[- ]?prescrib|contraindicat|interact|co[- ]?medicat)", re.I)
...
def infer_relation(q):
    ...
    if re.search(r"(conditions?|issues?|problems?)", q, re.I): return "ADVERSE_EFFECT"
...
def find_head_cui(question, surf2cui, surfaces):
    ...
    drugish = re.compile(r"(mab\b|cept\b|zumab\b|ximab\b|imab\b|umab\b|\bdrug\b|\btherapy\b|\btreatment\b)", re.I)
    best = None
    for s in surfaces:
        if s in qlow:
            cui = surf2cui[s]
            ...
            if drugish.search(question): break
    return best[1] if best else None

def main():
    if len(sys.argv) != 4:
        sys.stderr.write("Usage: python prep_autoparse_hints.py <in_jsonl> <dict_txt> <out_jsonl>\n")
        sys.exit(2)
