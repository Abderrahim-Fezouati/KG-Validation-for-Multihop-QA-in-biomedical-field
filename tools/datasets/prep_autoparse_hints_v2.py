...
AE_PAT  = re.compile(r"(adverse|side[- ]?effect|safety|harm|untoward|complication|toxicit|unfavorable|untoward)", re.I)
DDI_PAT = re.compile(r"(interact|co[- ]?prescrib|contraindicat|avoid(ed)? with|clash|co[- ]?medicat|serotonin syndrome|qt prolong)", re.I)

def infer_relation(q):
    if DDI_PAT.search(q): return "INTERACTS_WITH"
    if AE_PAT.search(q):  return "ADVERSE_EFFECT"
    # fallback
...
def load_kg_heads(kg_path):
    ...
    for nl, hid in sorted(labels, key=lambda x: (-len(x[0]), x[0])):
        if nl: lab2id.setdefault(nl, hid)
    return lab2id

def load_dict(dict_path):
    surf2id={}
    if not dict_path or not os.path.isfile(dict_path): return surf2id
...
def find_head(question, lab2id, dict2id):
    ...
    for n in range(min(6,len(tokens)),0,-1):  # try longer spans first
        for i in range(0,len(tokens)-n+1):
            ...
            if span in dict2id: return dict2id[span]
            if span in lab2id:  return lab2id[span]
    return None

def main():
    if len(sys.argv)<4:
        sys.stderr.write("Usage: python prep_autoparse_hints_v2.py <in_jsonl> <kg_csv> <out_jsonl> [umls_dict.txt]\n")
        sys.exit(2)
