...
VALID_PREFIXES = ("drug_", "disease_", "gene_", "chemical_", "procedure_", "exposure_", "food_", "device_", "organism_", "pathway_", "phenotype_", "symptom_")
AE_OR_DDI = {"ADVERSE_EFFECT", "INTERACTS_WITH"}

def looks_like_cui(x: str) -> bool:
    return bool(re.match(r"^(?:drug|disease|gene|chemical|procedure|exposure|food|device|organism|pathway|phenotype|symptom)_[a-z0-9_]+$", str(x or "")))
...
def best_cui_for_question(qtext: str, rels, overlay_map, dict_map):
    ...
    for surface, cui in dict_map.items():
        if not looks_like_cui(cui): continue
        if len(surface) < 3: continue
        if surface in qnorm:
    ...
    if any(r in AE_OR_DDI for r in (rels or [])):
        for _, _, cui in candidates:
            if cui.startswith("drug_"): return cui
...
def main():
    ...
    fixed = total = 0
    with io.open(args.in_path, "r", encoding="utf-8") as fin, \
         io.open(args.out_path, "w", encoding="utf-8", newline="\n") as fout:
        for line in fin:
            line = line.strip()
            ...
            if not looks_like_cui(head):
                inferred = best_cui_for_question(ex["text"], rels, overlay_map, dict_map)
                if inferred:
