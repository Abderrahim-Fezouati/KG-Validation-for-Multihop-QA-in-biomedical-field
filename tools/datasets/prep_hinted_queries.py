...
def load_dict(path):
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            cui  = parts[1].strip()
            if not surf or not cui: continue
...
def looks_like_cui(x):
    ...
    return bool(re.match(r"^(drug|disease|gene|chemical|procedure|exposure|food|device|organism|pathway|phenotype|symptom)_[a-z0-9_]+$", str(x)))
...
def main():
    ...
    try:
        ...
        print("Usage: python prep_hinted_queries.py --in in.jsonl --dict umls_dict.txt --out out.jsonl", file=sys.stderr)
        sys.exit(2)
        ...
        for line in fin:
            ...
            if "INTERACTS_WITH" in rels:
                if len(cuis) >= 2:
                    ...
                    ex["tail_cui"] = cuis[1] if cuis[1] != ex["head_cui"] else (cuis[2] if len(cuis) > 2 else None)
                elif len(cuis) == 1:
