...
# 4) STEP 2 – Intent detection + SapBERT EL (type-aware)
$analyzeScript = Join-Path $Root "scripts\03_analyze_queries_sapbert.py"
AbortIfNot (Test-Path $analyzeScript) "Analyzer script not found: $analyzeScript"
$out_analyzed = Join-Path $RUN "queries.analyzed.jsonl"

Write-Host "Running 03_analyze_queries_sapbert.py -> $out_analyzed"
