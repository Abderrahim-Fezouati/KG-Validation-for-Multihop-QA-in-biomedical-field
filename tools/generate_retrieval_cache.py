...
def main():
    ...
    if corpus_path is None:
        ...
        if os.path.exists(cand):
            ...
            print("ERROR: cannot infer corpus path. Please ensure data/corpus.jsonl exists or modify this script.")
            sys.exit(2)
    ...
    with open(args.out, "w", encoding="utf-8") as fout:
        for q in queries:
            ...
            top_sorted = [did for did,_ in sorted(scores.items(), key=lambda x: x[1], reverse=True)[:args.topk]]
            fout.write(json.dumps({"qid": qid, "hits": top_sorted}, ensure_ascii=False) + "\n")
